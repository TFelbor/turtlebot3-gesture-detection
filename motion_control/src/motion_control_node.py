...
class MotionNode:
    ...
    def move_robot(self):
        ...
        if self.current_gesture == "GO":
            ...
            twist.linear.x = 0.0
            twist.angular.z = 0.5  # Rotate Left (Counter-Clockwise)
            rospy.loginfo_throttle(1, "Turning LEFT")
            
        elif self.current_gesture == "RIGHT":
            twist.linear.x = 0.0
            twist.angular.z = -0.5 # Rotate Right (Clockwise)
            rospy.loginfo_throttle(1, "Turning RIGHT")
